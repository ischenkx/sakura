...
func (b *bucket) join(id string, ts int64, sessions []*session2.Session) {
	if len(sessions) == 0 {
		return
	}
	t := b.getOrCreate(id)
...
func (s *Storage) bucket(id string) *bucket {
	return s.buckets[util2.Hash(id)%len(s.buckets)]
}

func (s *Storage) Join(id string, ts int64, clients []*session2.Session) {
